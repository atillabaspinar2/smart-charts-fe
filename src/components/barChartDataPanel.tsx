...
export const BarChartDataPanel: FC<BarChartDataPanelProps> = ({
  ...
  const updateSeries = (
    seriesId: string,
    updater: (
      series: BarChartData["series"][number],
    ) => BarChartData["series"][number],
  ) => {
    onChange({
      ...data,
    ...
    onChange({
      ...
      series: [
        ...data.series,
        {
          ...
          values: Array.from({ length: data.categories.length || 5 }, () => 0),
        },
      ],
    });
  };
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-gray-500">
            Each series can have its own values and color.
          </p>
        </div>
        <button
          type="button"
      ...
      <div className="space-y-3">
        {data.series.map((series, index) => (
          ...
          >
            ...
            <CustomInput
              id={`bar-series-values-${series.id}`}
              label="Series Values"
              type="text"
              value={series.values.join(", ")}
              placeholder="5, 20, 36, 10, 10"
              onChange={(e) =>
                updateSeries(series.id, (current) => ({
                  ...current,
